...
{
    public class IncompleteProjectReasonsViewModel : ViewModelBase
    {
        bool isdirty = false;
        ...
        public IncompleteProjectReasonsViewModel()
        {        
            Save = new RelayCommand(ExecuteSave, CanExecuteSave);
            Cancel = new RelayCommand(ExecuteCancel, CanExecute);
            AddNew = new RelayCommand(ExecuteAddNew, CanExecuteAddNew);
            GetIncompleteProjectReasons();
            canexecutesave = false;
        
        }

        public FullyObservableCollection<ModelBaseVM> Reasons
        {
            get { return reasons; }
            set { SetField(ref reasons, value); }
        }
        ...
        private void GetIncompleteProjectReasons()
        {
            Reasons = GetReasonsForIncompleteProject();
            Reasons.ItemPropertyChanged += ProductNames_ItemPropertyChanged;
        }
        ...
        {
            int nummissing = Reasons.Where(x => string.IsNullOrEmpty(x.Name.Trim())).Count();
            return (nummissing > 0);
        }
        ...
        {
            Reasons.Add(new ModelBaseVM() {Name = string.Empty, IsEnabled = true, IsChecked = false });
        ...
        private void SaveAll()
        {
            if (isdirty)
            {
                foreach (ModelBaseVM item in Reasons)
                {
                    if (!string.IsNullOrEmpty(item.Name))
                    {
                        if (item.ID == 0)
                            item.ID = AddReasonForIncompleteProject(item);
                        else
                            UpdateReasonForIncompleteProject(item);
                    }
                }
                isdirty = false;
            }
